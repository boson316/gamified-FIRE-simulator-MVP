...
def _install_planning_assets(template_root: Path, project_root: Path, slo: dict) -> None:
    """Copy Senior planning docs, templates, and Cursor rule."""
    title = slo["project"]["title"]
    replacements = {"{{PROJECT_TITLE}}": title}
    ...
    planning_src = template_root / "PLANNING.md"
    if planning_src.is_file():
        (project_root / "PLANNING.md").write_text(
            planning_src.read_text(encoding="utf-8"), encoding="utf-8"
        )
    planning_en = template_root / "PLANNING.en.md"
    if planning_en.is_file():
        (project_root / "PLANNING.en.md").write_text(
            planning_en.read_text(encoding="utf-8"), encoding="utf-8"
        )

    templates_dir = template_root / "templates"
    for name in ("ROADMAP.md", "SUMMARY.md", "TECH_DEBT.md"):
        src = templates_dir / name
        if src.is_file():
            (project_root / name).write_text(_patch(src.read_text(encoding="utf-8")), encoding="utf-8")

    docs_dir = project_root / "docs"
    docs_dir.mkdir(parents=True, exist_ok=True)
    gtm = templates_dir / "go-to-market-checklist.md"
    if gtm.is_file():
        (docs_dir / "go-to-market-checklist.md").write_text(
            _patch(gtm.read_text(encoding="utf-8")), encoding="utf-8"
        )

    rule_src = template_root / "rules" / "senior-planning.mdc"
    if rule_src.is_file():
        rules_dst = project_root / ".cursor" / "rules"
        rules_dst.mkdir(parents=True, exist_ok=True)
        shutil.copy(rule_src, rules_dst / "senior-planning.mdc")
...
def main() -> int:
    ...
    _install_planning_assets(template_root, target_dir, slo)
    ...
    print("[NEXT] pytest -q")
    print("[NEXT] Read PLANNING.md + fill ROADMAP.md / TECH_DEBT.md")
