...
def render_agents_md(slo: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"- Senior 規劃：`PLANNING.md` · 雙指標見 `SUMMARY.md` / `ROADMAP.md`\n\n"
        "## Hard Constraints\n\n"
        f"- Python {project['python']}+\n"
        "- TDD：先改 `tests/`，再改 `src/`\n"
        "- 不得放寬測試語意硬過關\n"
        "- 高風險變更（routing/batch/retry/timeout）先跑 gate 再第二輪\n"
        "- 遵守 `PLANNING.md` 與 `.cursor/rules/senior-planning.mdc`\n"
        "- `go_to_market < 40%` 且 `code_complete ≥ 70%` 時預設不加 FR\n\n"
        "## Required Files\n\n"
        "- `PLANNING.md`, `ROADMAP.md`, `SUMMARY.md`, `TECH_DEBT.md`\n"
